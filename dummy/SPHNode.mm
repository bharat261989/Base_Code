...
bool QueryWorldInteractions::ReportFixture(b2Fixture* fixture) {
    ...
    for(int i = 0; i < numParticles; i++)
    {
        ...
        //This IF lets the water particles stay inside the tub only if they fall within the tub parameters
        ...
        //For particles nside the tub and collision happens
        else
        {
            b2Vec2 nearestPos(0,0);
            b2Vec2 normal(0,0);
           ...
           //     b2Vec2 buoyancy = -m_world->GetGravity();
              b2Vec2 buoyancy = b2Vec2(0, 1.0f);
                const float buoyancyAdjuster = 6.0f;
...
-(void)createStaticGeometry
{
    ...
    sd.SetAsBox(0.25f, 2.5f,b2Vec2(-3.0f,2.5f),0.0f);
    ground->CreateFixture(&gFix);
    
    //Right Block
    sd.SetAsBox(0.25f, 1.0f,b2Vec2(3.0f,1.2f),-0.5f);
    ground->CreateFixture(&gFix);
...
//    ground->CreateFixture(&gFix);
    ...
    //Delete the Right block
//    m_world->DestroyBody(sd).DestroyFixture(&gFix);
    
    //ground->DestroyFixture(<#b2Fixture *fixture#>);
    
    
    //This is the right block
//    sd.SetAsBox(0.25f, 1.0f,b2Vec2(4.0f,0.7f),0.0f);
//    ground->CreateFixture(&gFix);
    
//    sd.SetAsBox(0.5f, 4.0f,b2Vec2(0.0f,-3.0f),-1.55f);
//    ground->CreateFixture(&gFix);
    
    //Preethi: This is the outlet for the water burst
    sd.SetAsBox(0.5f, 0.25f,b2Vec2(-3.0f,5.0f),0.0f);
    ground->CreateFixture(&gFix);
    self.isTouchEnabled = YES;
    
    // Preethi:Box which falls down
...
//
//        //liquid[i].mPosition = b2Vec2(5.0f,5.0f);
...
//            cd.m_p = b2Vec2(-2.8f,3.0f);
    ...
    //Location162.000000 183.000000
    //194.000000 182.000000
    
    
    //If Else for conditional water
    if((location.x > 162.0 && location.x < 194) && (location.y > 182 &&  location.y<195)){
    
        if(ball_count == 0){
    NSLog(@"ball_count %d",ball_count);
    // Water Particles
	float massPerParticle = totalMass / nParticles;
...
-(void)applyLiquidConstraint:(float)deltaT
{
	...
	for(int i = 0; i < nParticles; i++)
	{
        ...
        //Preethi: For the fountain effect modify p
		float p = 0.0f;
		float pnear = 0.5f;
...
-(void)checkBounds
{
	...
	for (int i=0; i<nParticles; ++i)
	{
		...
		{
			float cx = -1.5f+ b2Random(0.0f, 0.7f) ;
			float cy = 0.5f  ;
